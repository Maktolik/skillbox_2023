...
class CatalogSerializer(serializers.ModelSerializer):
    ...
    def get_images(self, obj):
        result = [
            {"src": image.image_url, "alt": "Image description"}
            for image in ProductImage.objects.filter(item_id=obj.id)
        ]
        if result:
            return result
...
class SalesSerializer(serializers.ModelSerializer):
    ...
    def get_images(self, obj):
        result = [
            ...
            for image in ProductImage.objects.filter(item_id=obj.item.id)
        ]
        if result:
            return result
        return [{"src": "", "alt": "Image description"}]
...
class OrderProductDetailsSerializer(serializers.ModelSerializer):
    ...
    def get_images(self, obj):
        result = [
            {"src": image.image_url, "alt": "Image alt string"}
            for image in ProductImage.objects.filter(item_id=obj.item_id)
        ]
        if result:
            return result
        return [{"src": "", "alt": "Image alt string"}]
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def get_products(self, obj):
        items = OrderProduct.objects.select_related("item").filter(order_id=obj.id)
        serializer = OrderProductDetailsSerializer(items, many=True)
