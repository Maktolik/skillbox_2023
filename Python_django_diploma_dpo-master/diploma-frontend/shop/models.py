...
def item_image_directory_path(instance: "ProductImage", filename: str) -> str:
    return "products/productimage_{pk}/images/{filename}".format(
        pk=instance.pk, filename=filename
    )
...
class Category(models.Model):
    ...
    title = models.CharField(max_length=100, unique=True, verbose_name="название")
    image = models.ImageField(null=True, upload_to=category_icons_directory_path, verbose_name="иконка")
    ...
    def change_status(self):
        item = Product.objects.filter(category_id=self.pk, is_published=True)
        if item:
            self.active = True
            self.save()
        else:
...
class SubCategory(models.Model):
    ...
    image = models.ImageField(null=True, upload_to=subcategory_icons_directory_path, verbose_name="иконка")
    sort_index = models.PositiveIntegerField(default=0, verbose_name="индекс сортировки")
    active = models.BooleanField(default=False, verbose_name="статус активности")
    ...
    def change_status(self):
        item = Product.objects.filter(category_id=self.pk)
        if item:
            self.active = True
            self.save()
        else:
...
class Product(models.Model):
    ...
    preview = models.ImageField(null=True, blank=True, upload_to=product_preview_directory_path, verbose_name="просмотр")
    description = models.TextField(null=False, blank=True, default="", verbose_name="описание")
    ...
    shop = models.ForeignKey(Shop, null=False, on_delete=models.CASCADE, verbose_name="магазин")
    category = models.ForeignKey(Category, null=False, on_delete=models.CASCADE, verbose_name="категория товара")
    subcategory = models.ForeignKey(SubCategory, null=True, on_delete=models.CASCADE, verbose_name="подкатегория товара")
...
class Order(models.Model):
    ...
    totalCost = models.DecimalField(max_digits=8, decimal_places=2, verbose_name="полная стоимость")
...
class OrderProduct(models.Model):
    ...
    item = models.ForeignKey(Product, on_delete=models.PROTECT, verbose_name="позиция заказа")
...
class Payment(models.Model):
    ...
    payment = models.DecimalField(max_digits=8, decimal_places=2, verbose_name="сумма платежа")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="создано")
